...
with tab_map:
    ...
    if not _fly:
        m.fit_bounds([[4.5, -3.4], [11.3, 1.3]])  # tighter Ghana bbox — fills the frame
    Fullscreen().add_to(m)
  ...
  <div style="padding:12px 14px 10px 14px;">
    ...
    (function() {{
        function applyZoomVisibility(map) {{
            ...
            // MarkerCluster appends to the map's pane; find all cluster layers
        ...
        function tryAttach(attempts) {{
            ...
            if (maps.length) {{
                ...
                map.on('zoomend', function() {{ applyZoomVisibility(map); }});
            }} else if (attempts > 0) {{
                setTimeout(function() {{ tryAttach(attempts - 1); }}, 300);
            }}
        }}
        document.addEventListener('DOMContentLoaded', function() {{ tryAttach(20); }});
        // Also try immediately in case DOM is already ready
        if (document.readyState !== 'loading') {{ tryAttach(20); }}
    }})();
    </script>
    """
...
with tab_cluster:
    ...
    if len(geo_df) < min_cluster_size:
        ...
        if cluster_df.empty:
            ...
            # ── Cluster map ──
            with col_map:
                cm = folium.Map(location=[7.9465, -1.0232], zoom_start=7, tiles="CartoDB dark_matter", max_bounds=True, min_zoom=6)
